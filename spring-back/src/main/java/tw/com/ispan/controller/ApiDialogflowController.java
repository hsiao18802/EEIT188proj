...
public class ApiDialogflowController {
    ...
    public ResponseEntity<Map<String, Object>> handleCustomerSupportRequest(@RequestBody Map<String, Object> requestBody) {
        ...
        if (optionalMember.isPresent()) {
            ...
            if (existingRequest.isPresent()) {
                // 已經有未解決的請求，不需要創建新的	
                CustomerServiceRequest existing = existingRequest.get();
                existing.setIssueDescription(issueDescription != null ? issueDescription : "User re-requested for human agent support."); // 可選：更新問題描述
                customerServiceRequestRepository.save(existing);
            } else {
                // 沒有未解決的請求，創建新的
                CustomerServiceRequest customerServiceRequest = new CustomerServiceRequest();
 ...
 // 處理客服發送給會員的消息
    ...
    @PostMapping("/check-support-request")
    public ResponseEntity<Map<String, Boolean>> checkSupportRequest(@RequestBody Map<String, Integer> requestBody) {
        Integer membersId = requestBody.get("membersId");
        Optional<Members> member = membersRepository.findById(membersId);
        
        Map<String, Boolean> response = new HashMap<>();
        if (member.isPresent()) {
            // 查詢未解決的客服請求
            Optional<CustomerServiceRequest> existingRequest = customerServiceRequestRepository.findTopByMemberAndStatusOrderByCreatedAtDesc(member.get(), "Pending");
            response.put("exists", existingRequest.isPresent());
        } else {
